...
# +
sigma, u = (
    fem.Function(V, name="sigma", dtype=dtype),
...
def block_solver():
    ...
    if PETSc.Sys().hasExternalPackage("hypre") and not np.issubdtype(
        ...
        # to a H(curl) space
        V_H1 = fem.functionspace(
            msh, element("Lagrange", msh.basix_cell(), k, dtype=xdtype)
        )
        V_curl = fem.functionspace(
            msh, element("N1curl", msh.basix_cell(), k, dtype=xdtype)
        )
        ...
        if k == 1:
            ...
            cvec0.interpolate(
                lambda x: np.vstack((np.ones_like(x[0]), np.zeros_like(x[1])))
            )
            cvec1.interpolate(
                lambda x: np.vstack((np.zeros_like(x[0]), np.ones_like(x[1])))
            )
            pc_sigma.setHYPRESetEdgeConstantVectors(
                cvec0.x.petsc_vec, cvec1.x.petsc_vec, None
            )
        else:
...
for strategy_name, strategy in (
    ...
    if dolfinx.has_adios2:
        ...
        u.name = "u"
        with VTXWriter(
            msh.comm, f"output_mixed_poisson_{strategy_name}.bp", u, "bp4"
        ) as f:
            f.write(0.0)
# -
